			...
			<div class="tn-helper-clearfix">
				<div class="it-main2">
					...
					<div class="it-com-textnote">
						<span class="kuai"> 行业：<%=company.getViewNum()%></span> <span
							class="kuai"> 所在地：<%=company.getArea()%>
						</span> <span class="kuai"> 规模：<%=company.getSize()%>
						</span> <span class="kuai"> 性质：<%=company.getType()%>
						</span>
					</div>
					<div class="it-company-text">
						...
						<p class="p1" style="padding: 7px;">
							<strong> </strong> <span
								style="line-height: 3.5; font-size: 14px;"> <%=company.getBrief()%>
							</span>
						</p>
						<br />
			...
			<div class="it-post-box tn-border-dashed">
				<div class="it-post-name">
					<div class="tn-helper-right it-post-btn">
						<a class="it-font-underline" href="jobServlet?action=info&jobid=<%=job.getId() %>" target=_blank><span
					...
					<h3>
						<a title=对日Java软件开发工程师
							href="jobServlet?action=info&jobid=<%=job.getId() %>"
							target=_blank><%=job.getName()%></a>
					</h3>
				</div>
				<ul class="it-post">
